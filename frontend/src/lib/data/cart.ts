    ...
    try {
      const { cart } = await sdk.client.fetch<HttpTypes.StoreCartResponse>(
        `/store/carts/${id}`,
        {
          ...
          query: {
            fields:
              '*items,*region,*items.product,*items.variant,*items.variant.options,items.variant.options.option.title,*items.thumbnail,*items.metadata,+items.total,+shipping_methods.name'
          },
          headers,
          cache: 'no-cache'
        }
      ...
      // await removeCartId(); // CANNOT WRITE COOKIES IN SERVER COMPONENT during render
      return null;
    }
  }
}
...
export async function placeOrder(cartId?: string) {
  ...
  const completedOrderId = res?.data?.order?.id ?? res?.data?.order_set?.orders?.[0]?.id;

  if (completedOrderId) {
    revalidatePath('/user/orders');
    removeCartId();
    redirect(`/order/${completedOrderId}/confirmed`);
  }
